...
# Changed from multiprocessing.Manager().dict() to regular dict for Windows compatibility
status_response_cache = {}
evo_emotes = {
    "1": "909000063",   # AK
    "2": "909000068",   # SCAR
...
async def level_loop(team_code, key, iv, region):
    ...
    print(f"🎯 Level loop started for team code: {team_code}")
    
    while level_running:
        try:
            cycle_count += 1
            print(f"\n🔄 Level Cycle #{cycle_count} starting...")
            
            # Step 1: Join team (1 second)
            print(f"🎯 Joining team: {team_code}")
            join_packet = await GenJoinSquadsPacket(team_code, key, iv)
            await SEndPacKeT(whisper_writer, online_writer, 'OnLine', join_packet)
            await asyncio.sleep(1)  # 1 second delay after join
            
            # Step 2: Start match (instant)
            ...
            leave_packet = await ExiT(None, key, iv)
            await SEndPacKeT(whisper_writer, online_writer, 'OnLine', leave_packet)
            await asyncio.sleep(1)  # 1 second delay after leave
            
            print(f"✅ Level Cycle #{cycle_count} completed! (Total: ~30 seconds)")
            
            # Loop immediately continues to next cycle
            
        except Exception as e:
            print(f"❌ Error in level loop cycle #{cycle_count}: {e}")
            # Wait a bit before retrying
            await asyncio.sleep(2)
    
    print(f"🛑 Level loop stopped after {cycle_count} cycles")
...
"""
                        ...
                        # NEW LEVEL COMMAND - Combines join, start, wait, exit in 30s loop
                        if inPuTMsG.strip().startswith('/level '):
                            print('Processing level command in any chat type')
                            
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF0000]❌ ERROR! Usage: /level <teamcode>\nExample: /level ABC123\n\n[FFFF00]What it does:\n[FFFFFF]🎯 Join team - 1 second\n🎮 Start match - instant\n⏳ Wait - 28 seconds\n🚪 Leave team - 1 second\n🔄 Loop - immediately back to join\n[00FF00]Total Cycle: ~30 seconds\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                team_code = parts[1]
                                level_team_code = team_code
                                
                                # Stop any existing level task
                                if level_task and not level_task.done():
                                    level_running = False
                                    level_task.cancel()
                                    await asyncio.sleep(0.5)
                                
                                # Start new level task
                                level_running = True
                                level_task = asyncio.create_task(level_loop(team_code, key, iv, region))
                                
                                # SUCCESS MESSAGE
                                success_msg = f"""[B][C][00FF00]✅ LEVEL LOOP STARTED!

[FFFFFF]Team Code: [FFFF00]{team_code}

[00FF00]Cycle Pattern:
[FFFFFF]🎯 Join team - 1 second
🎮 Start match - instant
⏳ Wait - 28 seconds
🚪 Leave team - 1 second
...
[FF0000]To stop: /level_stop
"""
                                await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)

                        # STOP LEVEL COMMAND
                        if inPuTMsG.strip() == '/level_stop':
                            if level_task and not level_task.done():
                                level_running = False
                                level_task.cancel()
                                success_msg = f"[B][C][00FF00]✅ SUCCESS! Level loop stopped successfully!\n"
                                await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
                            else:
                                error_msg = f"[B][C][FF0000]❌ ERROR! No active level loop to stop!\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
...
03.[FFFF00]/level [code]-[FFFFFF]Auto level (30s loop)
04.[00FF00]/level_stop-[FFFFFF]Stop level loop
05.[FFFF00]/3-[FFFFFF]Send 3-player invite
06.[00FF00]/5-[FFFFFF]Send 5-player invite
07.[FF0000]/6-[FFFFFF]Send 6-player invite
08.[FFA500]! (team code)-[FFFFFF]Join squad
09.[FFA500]/ghost (team code)-[FFFFFF]Ghost join squad
10.[FF00FF]/e [emote]-[FFFFFF]Send emote to yourself
11.[00FF00]/e [uid] [emote]-[FFFFFF]Send emote to player
12.[FFA500]/e list-[FFFFFF]Show all emotes
13.[00FF00]/e list names-[FFFFFF]Show named emotes
"""
                            ...
                            emotes = """14.[FFC0CB][00FF00]@evos-[FFFFFF]Start evolution cycle
15.[00FF00]@sevos-[FFFFFF]Stop evolution cycle
16.[FFFF00]/evo [num]-[FFFFFF]Send evo emote (1-18)
17.[FFA500]/evo [uid] [num]-[FFFFFF]Send evo emote to player
18.[FF00FF]/fast [uid] [emote]-[FFFFFF]Fast emote spam
19.[00FF00]/p [uid] [emote] [num]-[FFFFFF]Custom emote spam
20.[00FF00]/reject [uid]-[FFFFFF]Reject spam
21.[FFA500]/reject_stop-[FFFFFF]Stop reject spam
"""
                            ...
                            spam_cmnd = """22.[00FF00]/s1 /s2 /s3 /s4 /s5-[FFFFFF]Spam with badges
23.[FF00FF]/bundle-[FFFFFF]Get Bundle list
24.[00FF00]/bundle [name]-[FFFFFF]Set Bundle 
25.[FFFF00]/msg [text] [times]-[FFFFFF]Message spam
26.[00FF00]/stop msg-[FFFFFF]Stop message spam
27.[FF0000]/mg [text] [repeats]-[FFFFFF]Wave message spam
28.[FF00FF]/inv [uid]-[FFFFFF]Send group invite
29.[FFFF00]/joinroom [id] [pass] -[FFFFFF]Join custom room
30.[FFA500]/lag [team_code]-[FFFFFF]Lag attack squad
31.[00FF00]/stop lag-[FFFFFF]Stop lag attack"""

                            await safe_send_message(response.Data.chat_type, spam_cmnd, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)

                            badge_cmnd = """32.[FFA500]/inv [uid]-[FFFFFF]Send group invite
33.[00FF00]/joinroom [id] [pas] [FFFFFF]-Join custom room
34.[00FF00]/lag [team_code]-[FFFFFF]Lag attack squad
35.[FF0000]/stop lag-[FFFFFF]Stop lag attack
36.[FFFF00]/info [uid]-[FFFFFF]Player information
37.[FF0000]/status [uid]-[FFFFFF]Check player status
38 [FFA500]/gali [name]-[FFFFFF]Gali spam message
39.[FFFF00]/likes [uid]-[FFFFFF]Send 100 likes
40.[00FF00]/ai [question]-[FFFFFF]Chat with AI
"""
                            ...
                            info_cmnd = """41.[00FF00]/wllist-[FFFFFF]View whitelisted UIDs
42.[FFA500]/wladd (uid)-[FFFFFF]Add UID to whitelist
43.[FF0000]/wlremove (uid)-[FFFFFF]Remove UID from whitelist
44.[FFFF00]/help-[FFFFFF]Show this help menu
45.[00FF00]/admin-[FFFFFF]Admin/contact information
46.[FFA500]/quick [cd] [em] [uid]-[FFFFFF]Quick emote
47.[FFFF00]/start-[FFFFFF]Start Chat Mode
48.[00FF00]/stop-[FFFFFF]Disable Chat Mode """
